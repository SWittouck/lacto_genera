...
# load data
load("results/parsed/lgc_protein.rda")
pairs <- read_csv("data/legen_v3_4/genome_pairs.csv.zip")
genes <- read_pangenome("data/legen_v3_4/pangenome_legen_v3_3/")

# add genome pairs (CNI scores etc) to the tidygenomes object
lgc_protein <-
  lgc_protein %>%
  add_tidygenomes(pairs) %>%
  add_tidygenomes(genes)
rm(pairs)
rm(genes)
save(lgc_protein, file = "results/parsed/lgc_withpairs.rda")
...
lgc_protein$nodes %>%
  ...
  left_join(lgc_protein$patterns) %>%
...
# exploration of signature genes that have some exceptions
lgc_protein$patterns <- NULL
lgc_protein$components <- NULL
lgc_protein <- add_orthogroup_measures(lgc_protein)
phylogroups <- lgc_protein$phylogroups$phylogroup
